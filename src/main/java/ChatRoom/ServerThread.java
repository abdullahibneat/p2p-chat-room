...
public class ServerThread extends Thread {
    ...
    public ServerThread(Client c) throws PortNotAvailbleException {
        ...
        try {
            ...
            client.postMessage("> Share your ADDRESS:PORT with other members: " + c.me.getAddress() + ":" + c.me.getPort());
        } catch (IOException e) {
...
class Handler implements Runnable {
    ...
    public void run() {
        try {
            ...
            if(obj instanceof String) {
                ...
                else client.postMessage(message); // Normal chat message
    ...
    private void newMember(String newMemberString) {
        ...
        if(id != client.me.getID()) {
            client.postMessage("New member \"" + newMemberArr[0] + "\" joined!");
    ...
    private void newCoordinator(int id) {
        for(Member m: client.getMembers()) {
            if(m.getID() == id) {
                m.setCoordinator();
                client.postMessage(m.getUsername() + " is the new coordinator!");
                break;
            }
        }
    }
